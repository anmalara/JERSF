...
class JERSFModule: public uhh2::AnalysisModule {

public:
  explicit JERSFModule(uhh2::Context&);
...
JERSFModule::JERSFModule(uhh2::Context & ctx) {
  ...
  if(isMC){ //L123 for MC
    if (jetLabel == "AK4CHS") {
      ...
      // else throw runtime_error("In JERSFModule.cxx: Invalid JEC_Version for deriving residuals on AK8PUPPI, MC specified ("+JEC_Version+") ");
    }
    else throw runtime_error("In JERSFModule.cxx: Invalid jetLabel: "+jetLabel+" ");
  }//MC
    ...
    if (jetLabel == "AK4CHS") {
      ...
      MAKE_JEC(Autumn18_V4, AK8PFPuppi)
      // else throw runtime_error("In JERSFModule.cxx: Invalid JEC_Version for deriving residuals on AK8PUPPI "+JEC_Version+", DATA specified.");
    }
    else throw runtime_error("In JERSFModule.cxx: Invalid jetLabel: "+jetLabel+" ");
...
JERSFModule::~JERSFModule() {

}

bool JERSFModule::process(Event & event) {
  ...
  if(event.isRealData){
    ...
    if(!(rl_event < upper_binborders_runnrs.back())){
      if(upper_binborders_runnrs.back() < rl_event) throw runtime_error("JERSFModule: run_lumi of event greater than largest bin-border.");
  ...
  if(apply_A+apply_B+apply_C+apply_D+apply_global != 1) throw runtime_error("In JERSFModule.cxx: Sum of apply_* when applying JECs is not == 1. Fix this.");
  ...
  if(isMC){
    ...
    for(Particle & genj : *event.genjets){
      ...
      else throw runtime_error("JERSFModule.cxx: Invalid jet-label specified.");
      int idx_g = 0;
...
UHH2_REGISTER_ANALYSIS_MODULE(JERSFModule)
