...
@timeit
def createConfigFiles(processes=["QCDPt15to30", "QCDPt15to30_MB", "DATA_RunF"], JECVersions_Data=["Autumn18_V4"], JECVersions_MC=["Autumn18_V4"], JetLabels=["AK4CHS"], systematics=["PU", "JEC", "JER"], original_dir = "./submittedJobs/", original_file = "JER2018.xml", outdir="JER2018", isMB = False, test_trigger=False, isThreshold=False, isLowPt=False, isL1Seed=False, isECAL=False, apply_PUid=False, extratext=""):
    ...
    for index_JEC, newJECVersion in enumerate(JECVersions_Data):
        for newJetLabel in JetLabels:
            ...
            for process in processes:
                ...
                if apply_PUid:
                    controls.append(["<!ENTITY", "APPLY_PUid", "false" , "true"])
                if "DATA" in process:
                    if "ECAL" in process:
