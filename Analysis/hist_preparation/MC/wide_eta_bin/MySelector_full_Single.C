          ...
          for ( int r = 0 ; r < EtaFtControlBinsNo ; r++ ){
            ...
            TMath::Abs( genjet1_eta ) < eta_bins_control[r+1]) ){
              for ( int m = 0 ; m < AlphaBinsNo ; m++ ){
                if ( alpha < alpha_bins[ m+1 ] ){
                  double gen_asy = gen_asymmetry;
                  if ((TMath::Abs( genjet2_eta ) > 0. && TMath::Abs( genjet2_eta ) < s_eta_barr && TMath::Abs( genjet1_eta ) > eta_bins_control[r] && TMath::Abs( genjet1_eta ) < eta_bins_control[r+1])) { gen_asy = - gen_asymmetry; }
          ...
          for ( int r = 0 ; r < EtaFtBinsNo ; r++ ){
            ...
            TMath::Abs( genjet1_eta ) < eta_ref_up[r]) ){
              for ( int m = 0 ; m < AlphaBinsNo ; m++ ){
                if ( alpha < alpha_bins[ m+1 ] ){
                  double gen_asy = gen_asymmetry;
                  if ((TMath::Abs( genjet2_eta ) > 0. && TMath::Abs( genjet2_eta ) < s_eta_barr && TMath::Abs( genjet1_eta ) > eta_ref_down[r] && TMath::Abs( genjet1_eta ) < eta_ref_up[r])) { gen_asy = - gen_asymmetry; }
