  ...
  for( int m = etaShift; m < etaBins+etaShift; m++ ) {
    ...
    for( int p = 0; p < ptBins; p++ ) {
      ...
      for( int r = 0; r < AlphaBins; r++ ) {
        ...
        TH2F *h2_MCTruth2D = new TH2F( name_MCTruth2D, name_MCTruth2D, 200, 0, 2.0, 200, 0, 2.0 );
        h2_MCTruth2D ->GetYaxis()->SetTitle("Response 1");    h2_MCTruth2D ->GetXaxis()->SetTitle("Response 2");
        h2_MCTruth2D -> Sumw2(); temp1_MCTruth2D.push_back(h2_MCTruth2D);
