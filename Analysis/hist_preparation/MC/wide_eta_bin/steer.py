          ...
          for sample in samples:
            ...
            if barrel_check>0:
              if barrel_check == 1:
                ...
                outdir = out_path_+pattern+sample+"barrel_check_1/"
              if barrel_check == 2:
                cmd = 'sed -i -e """s/s_eta_barr/0.783/g" MySelector.C'
                outdir = out_path_+pattern+sample+"barrel_check_2/"
              if barrel_check == 3:
                cmd = 'sed -i -e """s/s_eta_barr/1.131/g" MySelector.C'
                outdir = out_path_+pattern+sample+"barrel_check_3/"
              a = os.system(cmd)
              if not os.path.isdir(outdir):
                os.makedirs(outdir)
            cmd = "make clean"
