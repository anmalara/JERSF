...
void CorrectionObject::Derive_Thresholds_SiJet(bool pt_check, bool useHF){
  ...
  TTreeReaderValue<int> trg60_HF(myReader_DATA, "trigger60_HFJEC");
  TTreeReaderValue<int> trg80_HF(myReader_DATA, "trigger80_HFJEC");
  TTreeReaderValue<int> trg100_HF(myReader_DATA, "trigger100_HFJEC");
  TTreeReaderValue<int> trg160_HF(myReader_DATA, "trigger160_HFJEC");
  TTreeReaderValue<int> trg220_HF(myReader_DATA, "trigger220_HFJEC");
  TTreeReaderValue<int> trg300_HF(myReader_DATA, "trigger300_HFJEC");
  ...
  // double triggers_values         = {60,  80,   140,  200, 260, 320, 400, 450,  500};//
  double fitrange_down[n_trigger-1] = {30,  40,   70,   150, 220, 300, 360, 420,  470};
  double fitrange_up[n_trigger-1]   = {200, 140,  300,  350, 400, 450, 550, 1000, 950};

  std::cout << triggerVal.size() << " vs " << n_trigger << '\n';
  for (size_t i = 0; i < triggerVal.size(); i++) {
    std::cout << triggerVal[i] << '\n';
  }
  for(int i=0; i<n_trigger-1; i++){
    TString fitname = "fit";
    fitname +=  to_string(triggerVal[i+1]);
    func[i] = new TF1(fitname,SmoothFit, fitrange_down[i], fitrange_up[i],3);
    func[i]->SetParameters(triggerVal[i+1], 40., 1.);
    func[i]->SetParNames("p0", "p1", "N");
    double x1, x2;
    func[i]->GetRange(x1,x2);
    std::cout << triggerVal[i] << " " << x1 << " " << x2 << '\n';
    ptave_data_eff[i]->Fit(func[i],"R+");
    ptave_data_eff[i]->GetListOfFunctions()->Add(func[i]);
