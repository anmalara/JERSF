   ...
   for( int m = 0; m < etabin; m++ )
   {
      ...
      for( int p = 0; p < ptbin; p++ )
      {
         ...
         for( int r = 0; r < 6; r++ )
         {
            ...
            //         	std::cout << "hist name: " << temp1.at(r)->GetName() << std::endl;
            //         	std::cout << "Asym: fw" << name_fw << std::endl;
            //         	std::cout << "Asym: fwgen" << name_fwgen << std::endl;
         }
   ...
   for( int m = 0; m < etabin; m++ )
   {
      ...
      for( int p = 0; p < ptbin; p++ )
      {
         ...
         for( int r = 0; r < 6; r++ )
         {
            ...
            temp1.push_back( ( TH1F* )f.Get( name_fw ) );
            //				std::cout << "FE1: fw " << name_fw << std::endl;
            //std::cout << "FE1: fwgen " << name_fwgen << std::endl;
            //            std::cout << "saved to object, no of entries = " << (( TH1F* )f.Get( name_fwgen ))->GetEntries() << std::endl;
            if( data == false )
            {
               temp1gen.push_back( ( TH1F* )f.Get( name_fwgen ) );
               //temp1gen.push_back( ( TH1F* )f.Get( name_fw ) );
               //std::cout << "saved to object, name= " << name_fw << std::endl;
               //std::cout << "saved to object, no of entries = " << (( TH1F* )f.Get( name_fw ))->GetMean() << std::endl;
               //std::cout << "saved to object, name = " << name_fwgen << std::endl;
               //std::cout << "saved to object, no of entries = " << (( TH1F* )f.Get( name_fwgen ))->GetMean() << std::endl;
            }
         }
         temp2.push_back(temp1);
         if( data == false ) temp2gen.push_back(temp1gen);
...
{
   //	std::cout << "vector size level 1 " << Asymmetry.size() << std::endl;
   //std::cout << "vector size level 3 " << Asymmetry.at(0).at(0).size() << std::endl;
   //std::cout << "no of ent 1st hist: " << (*Asymmetry.at(0).at(0).at(0)).GetMean() << std::endl;

   for( unsigned int m = 0; m < Asymmetry.size(); m++ )
   {
      //      std::cout << "vector size level 2 " << Asymmetry.at(m).size() << std::endl;
      ...
      for( unsigned int p = 0; p < Asymmetry.at(m).size(); p++ )
      {
         ...
         {
            //				std::cout <<(*Asymmetry.at(m).at(p).at(r)).GetName() << std::endl;
