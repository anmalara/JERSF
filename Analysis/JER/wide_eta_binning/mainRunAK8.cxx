...
int mainRunAK8( bool data_, const char* filename, const char* filename_data, TString lumi, TString label_mc, TString label_data, TString Trigger, TString outdir, double gaustails = 0.985, float shiftForPLI = 0.0, int ref_shift = 3){
  ...
  if (Trigger.Contains("LowPt", TString::ECaseCompare::kIgnoreCase)) {
    ...
    for (int i = 0; i < n_pt_bins_Si; i++) Pt_bins_Central.push_back(pt_bins_SiAK8PUPPI[i]);
    for (int i = 0; i < n_pt_bins_Si_HF; i++) Pt_bins_HF.push_back(pt_bins_SiAK8PUPPI[i]);
  }
